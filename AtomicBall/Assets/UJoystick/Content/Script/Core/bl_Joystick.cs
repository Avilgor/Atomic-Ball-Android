...
public class bl_Joystick : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IDragHandler
{
    ...
    [Header("Settings")]
    [SerializeField, Range(0, 15)]private float Radio = 5;//the ratio of the circumference of the joystick
    ...
    public void OnDrag(PointerEventData data)
    {
        ...
        if (data.pointerId == lastId)
        {
            ...
            if (Vector3.Distance(DeathArea, position) < radio)
            {
    ...
    public float Horizontal
    {
        get
        {
            if (!isFree) return (StickRect.position.x - DeathArea.x) / Radio;
            else return 0;
    ...
    public float Vertical
    {
        get
        {
            if (!isFree) return (StickRect.position.y - DeathArea.y) / Radio;
            else return 0;
        }
    }
}